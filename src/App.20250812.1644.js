...
const Dashboard = ({ catId, onBack, showToast, user, theme, setTheme }) => {
    ...
    return (
        <div className="bg-gray-100 dark:bg-gray-900 min-h-screen font-sans">
            ...
            {(isAddingFood || foodToEdit) && <FoodFormModal onSave={handleSaveFood} onCancel={() => { setIsAddingFood(false); setFoodToEdit(null); }} initialData={foodToEdit} theme={theme} />}
...
const FoodFormModal = ({ onSave, onCancel, initialData, theme }) => {
    const [formData, setFormData] = useState({ name: initialData?.name || '', type: initialData?.type || 'mokra', calories: initialData?.calories || '' });
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20 p-4"><div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-md animate-fade-in-up"><form onSubmit={handleSubmit} className="space-y-4 text-gray-700 dark:text-gray-300"><div className="flex justify-between items-center"><h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">{initialData ? 'Edytuj karmę' : 'Dodaj nową karmę'}</h2><button type="button" onClick={onCancel} className="p-1 text-gray-400 hover:text-gray-600 rounded-full"><LucideX /></button></div><div><label className="block text-sm font-medium">Nazwa karmy</label><input type="text" name="name" value={formData.name} onChange={handleChange} className={inputClassName} required /></div><div><label className="block text-sm font-medium">Kaloryczność (kcal / 100g)</label><input type="number" name="calories" value={formData.calories} onChange={handleChange} className={inputClassName} required /></div><div><label className="block text-sm font-medium">Typ karmy</label><Select name="type" options={foodTypeOptions} value={foodTypeOptions.find(o => o.value === formData.type)} onChange={handleSelectChange} styles={customStyles} className="mt-1"/></div><div className="flex justify-end space-x-3 pt-4"><button type="button" onClick={onCancel} className="bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-800 dark:text-gray-200 font-bold py-2 px-4 rounded-lg transition">Anuluj</button><button type="submit" className="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg transition">{initialData ? 'Zapisz zmiany' : 'Dodaj karmę'}</button></div></form></div></div>);
};
...
const AddMealForm = ({ foods, onSave, onCancel, isEditMode, initialData, theme }) => {
    ...
    const foodOptions = useMemo(() =>
            foods.map(food => ({
                value: food.id,
                label: isEditMode ? food.name : `${food.name} (${food.calories} kcal/100g)`
            })),
        [foods, isEditMode]
    );
