...
// ZMIANA START: Dodanie hooka do obsługi zwijanych sekcji
...
const Dashboard = ({ catId, onBack, showToast, user, theme, setTheme }) => {
    ...
    // ZMIANA START: Inicjalizacja hooków dla zwijanych sekcji
    const profileCollapsible = useCollapsible();
    const weightCollapsible = useCollapsible();
    const toolsCollapsible = useCollapsible();
    const mealLogCollapsible = useCollapsible(true); // Dziennik posiłków domyślnie otwarty
    // ZMIANA KONIEC
            ...
            <main className="container mx-auto p-4 flex flex-col lg:grid lg:grid-cols-3 gap-6">
                <div className="order-2 lg:order-1 lg:col-span-1 flex flex-col gap-6">
                    <div className="order-3">
                        {isEditingProfile ? ( <CatProfileForm cat={cat} onSave={handleUpdateCat} onCancel={() => setIsEditingProfile(false)} theme={theme} onDeleteRequest={handleDeleteCatRequest} /> ) : (
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                                <div {...profileCollapsible.triggerProps}>
                                    ...
                                    <div className="flex items-center">
                                        <button onClick={(e) => { e.stopPropagation(); setIsEditingProfile(true); }} className="p-2 text-gray-500 dark:text-gray-400 hover:text-indigo-500 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition"><LucideClipboardEdit className="h-5 w-5" /></button>
                                        <LucideChevronDown className={`h-6 w-6 text-gray-500 dark:text-gray-400 transition-transform duration-300 lg:hidden ${profileCollapsible.isOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                </div>
                                <div {...profileCollapsible.contentProps}>
                    ...
                    <div className="order-2">
                        <WeightTracker catId={catId} user={user} showToast={showToast} collapsible={weightCollapsible} />
                    </div>

                    <div className="order-1">
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                            <div {...toolsCollapsible.triggerProps}>
                                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200 flex items-center"><LucideBone className="mr-2 h-6 w-6 text-indigo-500"/>Narzędzia</h2>
                                <LucideChevronDown className={`h-6 w-6 text-gray-500 dark:text-gray-400 transition-transform duration-300 lg:hidden ${toolsCollapsible.isOpen ? 'rotate-180' : ''}`} />
                            </div>
                            <div {...toolsCollapsible.contentProps}>
                                <div className="overflow-hidden">
                ...
                <div className="order-1 lg:order-2 lg:col-span-2 flex flex-col gap-6">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                        <div {...mealLogCollapsible.triggerProps}>
                            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200 flex items-center"><LucideCalendarDays className="mr-2 h-6 w-6 text-indigo-500"/> Dziennik Posiłków</h2>
                            <LucideChevronDown className={`h-6 w-6 text-gray-500 dark:text-gray-400 transition-transform duration-300 lg:hidden ${mealLogCollapsible.isOpen ? 'rotate-180' : ''}`} />
                        </div>
                        <div {...mealLogCollapsible.contentProps}>
                            <div className="overflow-hidden">
                                <div className="pt-4">
                                    <div className="flex items-center justify-between mb-4 bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg">
                                        <button onClick={() => handleDateChange(-1)} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition"><LucideChevronLeft className="h-5 w-5 text-gray-600 dark:text-gray-300" /></button>
                                        <span className="font-semibold text-gray-700 dark:text-gray-300 text-center">{new Date(currentDate + 'T00:00:00').toLocaleDateString('pl-PL', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                        <button onClick={() => handleDateChange(1)} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition"><LucideChevronRight className="h-5 w-5 text-gray-600 dark:text-gray-300" /></button>
                                    </div>
...
// ZMIANA: Dodano prop `collapsible`
const WeightTracker = ({ catId, user, showToast, collapsible }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
            <div {...collapsible.triggerProps}>
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200 flex items-center"><LucideLineChart className="mr-2 h-6 w-6 text-indigo-500"/> Historia wagi</h2>
...
const StatisticsModal = ({ catId, user, onCancel }) => {
    ...
    const [loading, setLoading] = useState(true);
    const [timeRange, setTimeRange] = useState(30); // Domyślnie 30 dni
    const catsPath = userCatsCollectionPath(user.uid);
    ...
    useEffect(() => {
        const fetchStats = async () => {
            ...
            const allMeals = [];
            let daysWithMeals = 0; // Licznik dni, w których kot jadł

            for (let i = 0; i < timeRange; i++) { // Pętla uwzględnia wybrany zakres
                const date = new Date();
                date.setDate(date.getDate() - i);
                const dateString = date.toISOString().split('T')[0];
                const mealDoc = await getDoc(doc(mealsCol, dateString));

                // Sprawdzamy, czy dokument istnieje i zawiera posiłki
                if (mealDoc.exists() && mealDoc.data().meals && mealDoc.data().meals.length > 0) {
                    allMeals.push(...mealDoc.data().meals);
                    daysWithMeals++; // Zwiększamy licznik tylko, gdy były posiłki
                }
            }
            ...
            // Nowa, precyzyjna kalkulacja średniej
        ...
        fetchStats();
    }, [catId, catsPath, timeRange]); // Dodajemy timeRange do zależności, by przeliczyć statystyki po zmianie
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20 p-4">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-2xl animate-fade-in-up flex flex-col" style={{maxHeight: '90vh'}}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">Statystyki z ostatnich {timeRange} dni</h2>
                    <button type="button" onClick={onCancel} className="p-1 text-gray-400 hover:text-gray-600 rounded-full"><LucideX /></button>
                </div>

                {/* Przyciski do zmiany zakresu czasu */}
                        ...
                        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            <h3 className="font-bold text-gray-700 dark:text-gray-300 mb-2">
                                ...
                                <Tippy content="Obliczane tylko z dni, w których podano posiłek.">
                                    <span className="text-xs font-normal text-gray-500 dark:text-gray-400 ml-1 cursor-help">(?)</span>
                                </Tippy>
                            </h3>
...
// ZMIANA: Komponent dostosowany do bycia zwijanym
const DailyHealthLog = ({ catId, user, currentDate, initialData, showToast }) => {
    ...
    if (!isEditing) {
        return (
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                <div {...collapsible.triggerProps}>
                    <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200 flex items-center"><LucideNotebookText className="mr-2 h-6 w-6 text-indigo-500"/> Dziennik Zdrowia</h2>
                    <LucideChevronDown className={`h-6 w-6 text-gray-500 dark:text-gray-400 transition-transform duration-300 lg:hidden ${collapsible.isOpen ? 'rotate-180' : ''}`} />
                </div>
                <div {...collapsible.contentProps}>
                    <div className="overflow-hidden">
                        <div className="pt-4 space-y-3 text-sm text-gray-700 dark:text-gray-300 cursor-pointer lg:cursor-auto" onClick={() => setIsEditing(true)}>
...
// Pozostałe komponenty (LabResultsModal, CatProfileForm, etc.) pozostają bez zmian.
// ... wklej tutaj pozostałe niezmienione komponenty, tak jak w poprzednich odpowiedziach ...
// (LabResultsModal, CatProfileForm, DeleteCatModal, FoodFormModal, AddMealForm, ConfirmationModal, FoodManagementModal, ExportModal)
...
const AddMealForm = ({ foods, onSave, onCancel, isEditMode, initialData, theme }) => {
    ...
    const CustomOption = ({ innerProps, label, data }) => {
        const content = (
            <div className={`flex items-center p-2 cursor-pointer ${isDark ? 'hover:bg-gray-600' : 'hover:bg-gray-100'}`}>
        ...
        if (data.photoURL) {
            return (
                <Tippy
                    content={<img src={data.photoURL} alt={`Powiększenie ${label}`} className="w-36 h-36 object-contain rounded-md" />}
                    placement="left"
        ...
        return <div {...innerProps}>{content}</div>;
    };
    ...
    const CustomSingleValue = ({ children, ...props }) => {
        ...
        if (data.photoURL) {
            return (
                <Tippy
                    content={<img src={data.photoURL} alt={`Powiększenie ${children}`} className="w-36 h-36 object-contain rounded-md" />}
                    placement="top"
                    animation="scale-subtle"
                    delay={[100, 0]}
                >
