    ...
    return (
        <div
            ...
            <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
                ...
                <h2 className="text-xl text-center text-gray-600 dark:text-gray-300 mb-8">Wybierz profil lub stwórz
                    nowy</h2>

                {cats.length > 0 && (
                    <div className="space-y-3 mb-6">
                        {cats.map(cat => (
                            <button key={cat.id} onClick={() => navigate(`/dashboard/${cat.id}`)}
                                    className="w-full flex items-center text-left bg-gray-100 dark:bg-gray-700 hover:bg-indigo-100 dark:hover:bg-indigo-900 p-4 rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                ...
                                <span
                                    className="text-lg font-semibold text-gray-800 dark:text-gray-200 ml-4">{cat.name}</span>
                ...
                {isCreatingCat ? (
                    <CatProfileForm onSubmit={handleCreateCat} onCancel={() => setIsCreatingCat(false)} theme={theme}/>
                ) : (
                    <button onClick={() => setIsCreatingCat(true)}
                            className="w-full flex items-center justify-center bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg transition-all duration-300">
                        <LucidePlusCircle className="mr-2 h-5 w-5"/>
                        Stwórz Nowy Profil Kota
                    </button>
                )}
                <button onClick={() => signOut(auth)}
                        className="w-full mt-4 text-sm text-gray-500 hover:text-indigo-500 flex items-center justify-center">
