...
const FoodManagementModal = ({foods, hiddenFoodIds, onToggleHide, onCancel, onEdit, onDelete}) => {
    ...
    const ImageWithTooltip = ({food}) => {
        const imageContent = (
            <div
                className="w-12 h-12 bg-gray-200 dark:bg-gray-600 rounded-md flex items-center justify-center flex-shrink-0">
                {food.photoURL ? (
        ...
        if (food.photoURL) {
            return (
                <Tippy
                    content={<img src={food.photoURL} alt={`Powiększenie ${food.name}`}
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20 p-4">
                                ...
                                <div key={food.id}
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500 dark:text-gray-400">{food.type}, {food.calories} kcal/100g {!isOwner && food.ownerId &&
                                                <span className='italic'>(Inny użytkownik)</span>}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center space-x-1">
                                        {isOwner && <button onClick={() => onEdit(food)}
                                                            className="p-2 text-gray-500 dark:text-gray-400 hover:text-indigo-500 rounded-full hover:bg-indigo-100 dark:hover:bg-gray-600 transition"
                                                            title="Edytuj"><LucideClipboardEdit className="h-5 w-5"/>
                                        </button>}
                                        {isOwner && <button onClick={() => handleDeleteRequest(food.id)}
                                                            className="p-2 text-gray-500 dark:text-gray-400 hover:text-red-500 rounded-full hover:bg-red-100 dark:hover:bg-gray-600 transition"
                                                            title="Usuń"><LucideTrash2 className="h-5 w-5"/></button>}
                                        <button onClick={() => onToggleHide(food.id, isHidden)}
                                                className="p-2 text-gray-500 dark:text-gray-400 hover:text-blue-500 rounded-full hover:bg-blue-100 dark:hover:bg-gray-600 transition"
                                                title={isHidden ? "Odkryj" : "Ukryj"}>
                                            {isHidden ? <LucideEye className="h-5 w-5"/> :
            ...
            {confirmingDelete &&
                <ConfirmationModal message="Czy na pewno chcesz usunąć tę karmę? Tej operacji nie można cofnąć."
                                   onConfirm={confirmDelete} onCancel={() => setConfirmingDelete(null)}/>}
        </div>
    );
};
